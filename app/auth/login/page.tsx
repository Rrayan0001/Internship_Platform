  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) {
      if (error.message === 'Invalid login credentials') {
        // Check if this is a Google-only account by probing with OTP (no user creation)
        const { error: otpError } = await supabase.auth.signInWithOtp({
          email,
          options: { shouldCreateUser: false },
        })
        // If OTP succeeds, user exists but has no password → Google account
        if (!otpError) {
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {isGoogleAccount && (
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm">
                  ...
                  <span>
                    This account was created with Google. Please{' '}
                    <button
                      type="button"
                      onClick={handleGoogleLogin}
